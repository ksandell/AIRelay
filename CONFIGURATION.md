...
| Var | Default | Notes |
|---|---|---|
| `PROXY_PROVIDER` | `generic` | One of: `anthropic`, `openai`, `google`, `mistral`, `groq`, `microsoft`, `openrouter`, `together`, `fireworks`, `deepseek`, `xai`, `perplexity`, `ollama`, `nvidia`, `generic`. Selects the response parser and the pricing table key. `generic` records bytes only (no token/cost fields). |
